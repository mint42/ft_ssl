...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sha512_get_options.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: rreedy <marvin@42.fr>                      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/17 10:38:54 by rreedy            #+#    #+#             */
/*   Updated: 2019/10/20 05:07:00 by rreedy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static const char	*valid_ops[SHA512_TOTAL_VALID_OPS + 1] = {
		"-p",
		"-q",
		"-r",
		"-s",
	...
	*option = 0;
	while ((*option) < SHA512_TOTAL_VALID_OPS)
	{
...
int				sha512_get_options(int argc, char **argv, int *argv_index, struct s_input *input)
{
	...
	while (*argv_index < argc && argv[*argv_index][0] == '-')
	{
		...
		{
			ft_printf("ft_ssl: sha512: '%s' is an invalid option", argv[*argv_index]);
			ft_queue_del(&(input->args), ft_queue_del_content);
			return (ERROR);
		}
		input->opts = (input->opts) | (1 << option);
		if (option == (SHA512_OP_P | SHA512_OP_S))
